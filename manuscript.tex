...
\section{2. Setting Up a Collaborative Work space (Author:Sarah jasmin
Klausen)}\label{setting-up-a-collaborative-work-space-authorsarah-jasmin-klausen}

Alright! Now that we understand the importance of having a robust and
reproducible scientific workflow that allows for collaboration we can
start by setting up a collaborative workspace and understanding some key
terms. In this tutorial we assume that you will be working with RStudio
and have already downloaded R and RSTudio. In addition you need to have
the version control software Git installed and have an account in
GitHub. If you need guidance for this you can find a helpful tutorial
\href{https://dhammarstrom.github.io/r-crash-course/01-software.html}{here}.

\subsubsection{What is a repository?}\label{what-is-a-repository}

A repository is like a project box where you collect all the files,
data, graphs and code scripts from your project. Online repositories can
be accessed from the internet and from any computer, while a local
repository is only stored in a specific computer and cannot be accessed
elsewhere. When setting up a collaborative work space it is advantageous
to have an online repository so that multiple people can contribute from
their own computer to a shared repository, without having to send files
by mail etc. In addition we can connect the online repository with a
local repository which allows us to work and make changes using our own
computer while still keeping the online repository up to date.

\subsubsection{What is Git?}\label{what-is-git}

Git is a version control software that allows you to track the different
version of your files. It allows you to keep a detailed history of
changes you have done in your document and also what other people have
added or removed in your collaborative documents. Having a version
controls software set up for your workflow is very handy as it prevents
major losses of documents and changes, and if any error is introduced in
a document or code, you can track it back to see what and who submitted
it. This fosters reproducibility, transparency, collaboration and
robustness for your project.

\subsubsection{What is GitHub?}\label{what-is-github}

GitHub is a collaborative online platform that allows you to host and
join online repositories. It is kinda like facebook for coding. GitHub
allows us to share and collaborate with other people on the same code at
the same time. It can also be used to host webpages and other fun
things.

In this tutorial we will only work with the RStudio interface and the
online GitHub interface. However, if you want an expanded command line
and interface for GitHub you can use GitHUb CLI and/or GitHub Desktop.
See tutorials here:
\href{https://docs.github.com/en/github-cli/github-cli/quickstart}{GitHub
CLI} and
\href{https://docs.github.com/en/desktop/overview/getting-started-with-github-desktop}{GitHub
Desktop}.
...
an online repository, you can essentially go about it to ways.\\
a) You can create the online repository and then clone it down to your
computer.\\
...
GitHub (\href{https://www.youtube.com/watch?v=f26KI43FK58}{Video
...
\item
  Navigate to the top-right corner of the page and click the + dropdown
  menu.
\item
  Select New repository from the dropdown options.
\item
  On the repository creation page, do the following:
\end{enumerate}
...
\item
  Add a description (optional): Briefly explain what your repository
  will contain.
\item
  Set visibility: Choose whether the repository will be Public (visible
...
\item
  A .gitignore file: Specify which files or directories Git should
  ignore.
\item
  A license: Define the terms of use for your repository.
\end{itemize}

However its also possible to create these documents after creating the
repository.
...
A README file is a descriptive file that should explain what the project
or repository is about, how it is organized and what the data in it
...
your code, but just your code. It´s content could look like this:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{.Rproj.user}
...
\ExtensionTok{/manuscript\_files/}
\end{Highlighting}
\end{Shaded}

\paragraph{Licenses}\label{licenses}

If you plan to share your scientific work, there are certain
intellectual property rights that can go along with it. In terms of
reproducibility and transparency in science we want our work to be
shared and used, but we also want people to acknowledge the authors who
did the original work. That´s where licenses come in. There are many
complete licences that you can add to your project and documents. For
open-source scientific work the \textbf{Creative Commons (CC BY)}
license is a good option. You can find it here
(\href{https://creativecommons.org/}{Creative Commons license}) or
choose another license that fits your project at
\href{https://choosealicense.com/}{choosealicense.com}.

To add a license to your project you can either do it when creating the
repository, or you can do it manually later. If you choose to do it
manually you can follow these simple steps:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Choose a license that fits your project and copy its entire text
\item
  Create a new file in the root of your project directory and name it
  \textbf{license} or \textbf{license.txt}.
\item
  Add the license text to your new file and save.
\end{enumerate}

Now your project has an associated license. If you also want to add the
license info to you metadata in your documents you can include it in
your YAML headings like this:

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{{-}{-}{-}} 
\ExtensionTok{title:} \StringTok{"My quarto document"}
\ExtensionTok{author:} \StringTok{"your name"}
\ExtensionTok{date:} \StringTok{"2025{-}12{-}24"}
\ExtensionTok{format:}\NormalTok{ pdf}
...
connect it to your local computer. You can do this multiple ways, but in
this tutorial we´ll use the GitHub and RStudio interface to make our
life easier.
...
\item
  Click the green \textbf{\textcolor{green}{Code}} button.
\item
...
Now, what if you wanted to do it the other way around, for example if
you already have a local project on your computer and want to create an
...
If you refresh the page of your online repository in GitHub you should
see a copy of the local project in the repository you just created.
...
can start pushing some code! But before we jump into the commands for
...
The \textbf{index/staging area} is a list of files that Git is tracking
for changes. When you ``stage'' files you tell Git to include these
...
\subsubsection{2.2.1 Commands}\label{commands}

Okay, now we can take a look at the common commands we use in this
workflow.
...
to stage. Remember to also include the file extensions (like .txt or
.pdf etc.)
...
Now lets say your colleague, who you are collaborating with, has added a
new file to the online repository on GitHub. You want to pull that
document from the online directory down to your local repository and
...
This command pulls the latest changes from your online repository and
...
\textbf{\texttt{git\ log}} will give you a list of the commits that are
not in your local branch (\textbf{HEAD}) together with their metadata
(authors, date etc.). This is helpful if you want to quickly answer
``What was changed?'' and ``Who changed it?''.

\textbf{\texttt{git\ diff}} will show you the actual changes that are
between two points, ex. your local branch (\textbf{HEAD}) and your
fetched branch (\textbf{origin/main}). \texttt{git\ diff} will show you
the specific lines of code that were added, modified or deleted in the
fetched branch. This is helpful if you want to know exactly what was
changed.

Then, if you want to integrate the fetched branch with your current
...
\subsubsection{2.2.2 Branches}\label{branches}
...
resulting output. \href{https://sikt.no/tjenester/sikt-ki}{Link to AI
model}.
...
\NormalTok{humac.dat }\OtherTok{\textless{}{-}} \FunctionTok{read\_excel}\NormalTok{(}\StringTok{"data/relief\_humac.xlsx"}\NormalTok{, }
    \AttributeTok{na =} \StringTok{"na"}\NormalTok{) }\CommentTok{\# this is the peak torque test data}

\NormalTok{condition }\OtherTok{\textless{}{-}} \FunctionTok{read\_excel}\NormalTok{(}\StringTok{"data/relief\_volume.xlsx"}\NormalTok{, }
    \AttributeTok{na =} \StringTok{"na"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# this is info about volume condition}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{participant =} \FunctionTok{as.character}\NormalTok{(participant))}

\NormalTok{participants }\OtherTok{\textless{}{-}} \FunctionTok{read\_excel}\NormalTok{(}\StringTok{"data/relief\_participants.xlsx"}\NormalTok{, }
    \AttributeTok{na =} \StringTok{"na"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# this is info about age group and allocation}
